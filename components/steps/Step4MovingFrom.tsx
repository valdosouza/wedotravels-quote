...
export default function Step4MovingFrom({ data, onChange, onNext, onBack }: Props) {
  ...
  const canContinue = from.eircode.trim().length >= 3 && from.address.trim().length >= 3 && from.city.trim().length >= 2;

  return (
    <div className="animate-in">
      <ProgressBar currentStep={4} totalSteps={7} />

      <h2 className="text-2xl font-bold text-white mb-2">Move details</h2>
      <p className="text-slate-400 text-sm mb-6">
        Add the collection and delivery details, then any important notes for the move.
      </p>

      <div className="quote-card mb-6">
        <div className="flex items-center gap-2 mb-4">
          <span className="text-lg">📍</span>
          <p className="text-white font-semibold">Moving from</p>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <EircodeInput
              value={from.eircode}
              onChange={(val) => updateFrom({ eircode: val })}
              onLookup={({ address, city, county }) =>
                updateFrom({ address, city, county })
              }
