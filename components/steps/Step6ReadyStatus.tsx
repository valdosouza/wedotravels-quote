...
export default function Step6ReadyStatus({ data, onChange, onNext, onBack }: Props) {
  ...
  return (
    <div className="animate-in">
      ...
      <h2 className="text-2xl font-bold text-white mb-2">How ready is the move right now?</h2>
      <p className="text-slate-400 text-sm mb-6">
        Choose the closest stage of the move.
      </p>
      ...
      <div className="space-y-2 mb-5">
        {PACKING_OPTIONS.map((opt) => (
          <button
            key={opt.value}
            className="w-full text-left px-5 py-4 rounded-xl text-white font-medium text-sm transition-all duration-150"
            onClick={() => onChange({ packingStatus: opt.value })}
            style={{
              backgroundColor:
                data.packingStatus === opt.value
                  ? "rgba(59,130,246,0.15)"
                  : "#0a1628",
              border: `1.5px solid ${data.packingStatus === opt.value ? "#3b82f6" : "#1e3a5f"}`,
            }}
          >
            {opt.label}
          </button>
      ...
      {/* Needs ride */}
      <div
        className="quote-card mb-5 cursor-pointer"
        onClick={() => onChange({ needsRide: !data.needsRide })}
      >
        <label className="flex items-start gap-3 cursor-pointer">
          <input
            type="checkbox"
            className="quote-checkbox mt-0.5"
            checked={data.needsRide}
